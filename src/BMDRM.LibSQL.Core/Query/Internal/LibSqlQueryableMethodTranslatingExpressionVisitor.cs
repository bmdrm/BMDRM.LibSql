...
public class LibSqlQueryableMethodTranslatingExpressionVisitor : RelationalQueryableMethodTranslatingExpressionVisitor
{
        ...
        : base(dependencies, relationalDependencies, queryCompilationContext)
    {
        ...
        _areJsonFunctionsSupported = new Version(new HttpDbConnection().ServerVersion) >= new Version(3, 38);
    }
    ...
    protected override ShapedQueryExpression? TranslatePrimitiveCollection(
        ...
        string tableAlias)
    {
        // Support for JSON functions (e.g. json_each) was added in LibSql 3.38.0 (2022-02-22, see https://www.LibSql.org/json1.html).
        // This determines whether we have json_each, which is needed to query into JSON columns.
        if (!_areJsonFunctionsSupported)
        {
            AddTranslationErrorDetails(LibSqlStrings.QueryingIntoJsonCollectionsNotSupported(new HttpDbConnection().ServerVersion));
