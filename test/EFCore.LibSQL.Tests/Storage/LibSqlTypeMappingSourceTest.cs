...
public class LibSqlTypeMappingSourceTest : RelationalTypeMappingSourceTestBase
{
    ...
    public void Throws_for_unrecognized_property_types()
    {
        ...
        var entityTypeBuilder = modelBuilder.Entity("Entity1");
        var propertyBuilder = entityTypeBuilder.Property(typeof(object), "Strange");
        var property = (IProperty)propertyBuilder.Metadata;

        var ex = Assert.Throws<InvalidOperationException>(
            () => CreateRelationalTypeMappingSource(CreateModel()).GetMapping(property));
        Assert.Equal(
            RelationalStrings.UnsupportedPropertyType(((IEntityType)property.DeclaringType).DisplayName(), "Strange", "object"), ex.Message);
    ...
    {
        var typeMappingSource = new LibSqlTypeMappingSource(
